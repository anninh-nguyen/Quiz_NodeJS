...
async function loadQuestions(keyword = "", page = 1) {  
  ...
  try {
    ...
    const solvedCount = questions[0]?.filter((q) => q[CONFIG.API_FIELDS.SOLVED]).length;
    ...
    let html = `
      <div class="score-bar">
        ...
        <div class="score-item">
          <div class="score-value">${solvedCount}/${total}</div>
...
async function loadQuestionDetail(qId) {
  ...
  try {
    const question = await apiFetch(`${CONFIG.ROUTES.QUESTIONS}/${qId}`);
    const currentUserId = getCurrentUserId();
...
async function playQuestion(qId) {
  ...
  try {
        ...
        ${
          question.keywords && question.keywords.length
            ? `<div class="question-keywords" style="justify-content:center;margin-bottom:1.5rem">
                  ${question.keywords.map((k) => `<span class="keyword">${k.name}</span>`).join("")}</div>`
            : ""
        }
    ...
    document.getElementById("play-form").addEventListener("submit", async (e) => {
      ...
      try {
        const result = await apiFetch(`${CONFIG.ROUTES.QUESTIONS}/${qId}/answer`, {
          method: "POST",
          body: JSON.stringify({ answered: answer }),
        });
